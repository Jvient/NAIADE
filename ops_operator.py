...
class ObsNetwork:
    ...
    def add_argo(self, n=15, **kw):
        """Flotteurs : cycle de ~10 j, position aléatoire dans une cellule."""
        spec = Platform(**{**PRESETS["argo"], **kw})
        step = max(1, int(round(spec.period_days)))
        c = max(1, spec.cell_px)
        for _ in range(n):
            cx, cy = self._rand_ocean_pos()
            times = np.arange(int(self.rng.integers(0, step)), self.nt, step)
            xs, ys = [], []
            for _t in times:
            ...
            self._new_sensor(spec, times, xs, ys)
        return self

    def add_drifters(self, n=10, u=None, v=None, **kw):
        """Dériveurs advectés. u, v : champs de vitesse (nx, ny) en px/jour,
        ou scalaires. Sinon marche aléatoire pure."""
